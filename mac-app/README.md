...
- **对话文件面板**（⇧⌘F / 顶部右侧图标，可收回）：跟随 GUI 当前对话，只显示该对话的**交付文件**——来源是会话目录下的交付清单 `delivered.json`，没有清单即显示「本次对话暂无交付文件」
- 自动管理后台服务生命周期：
...
「对话文件面板」只显示**交付清单**里的文件。约定：助手在对话中交付成果文件时，把文件登记到该会话目录的清单：

```
~/.dsh/sessions/<项目目录>/<会话ID>/delivered.json
# 内容:{"files": ["相对工作区的路径", ...]}   (支持绝对路径)
```

没有登记 = 没有交付，面板显示「本次对话暂无交付文件」。工作过程文件（源码、脚本、配置等）不要登记。
